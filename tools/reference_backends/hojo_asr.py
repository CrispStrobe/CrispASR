...
Memory
------
The merged checkpoint is 11.96 GB (encoder + adapter F32, Qwen3-4B BF16). On a
16 GB CPU box `HOJO_ASR.load_model` fits because safetensors loads with
assign=True (one copy, not two). It does NOT fit alongside a second copy, so
this dumper never clones the model. Set HOJO_ASR_DEVICE=cuda to use a GPU.
...
def dump(*, model_dir: Path, audio: np.ndarray, stages: Set[str],
    ...
    # being compared against the wrong tensor.
    ref_speech = _np(speech_embeddings).reshape(-1, speech_embeddings.shape[-1])
    if "speech_embeds" in out:
        delta = float(np.max(np.abs(out["speech_embeds"] - ref_speech)))
        if delta > 1e-4:
