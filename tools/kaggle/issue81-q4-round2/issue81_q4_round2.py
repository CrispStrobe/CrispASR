...
A/B tests direct subsampler convolutions, per-head-mask FlashAttention and
selective FFN precision on P100. Each arm uses the same varied audio and hard
transcript stability/equality gates.
"""
...
REPO = TEMP / "CrispASR"
BUILD = TEMP / "build"
BRANCH = "bench/81-q4-direct-conv"
SCRIPT_VERSION = "issue81-round2-v1"
...
with kh.build_heartbeat("cmake.build"):
    kh.sh_with_progress(
        f"cmake --build {BUILD} -j {kh.safe_build_jobs(True)} --target crispasr-lib crispasr-quantize"
    )
...
models = {
    ...
    "ctc_f16": hf_hub_download(
        "cstr/parakeet-ctc-0.6b-GGUF", "parakeet-ctc-0.6b.gguf",
        local_dir=str(models_dir), token=token,
    ),
}
quant = BUILD / "bin" / "crispasr-quantize"
ffn_rule = r"encoder\.layers\..*\.ff[12]\.linear[12]\.weight"
for precision in ("q8_0", "f16"):
    out = models_dir / f"parakeet-ctc-0.6b-q4_k-ffn-{precision}.gguf"
    subprocess.check_call([str(quant), models["ctc_f16"], str(out), "q4_k",
                           "--tensor-type", f"{ffn_rule}={precision}"])
    models[f"ctc_ffn_{precision}"] = str(out)
kh.step("models.downloaded_and_hybrids_built",
        sizes={k: Path(v).stat().st_size for k, v in models.items()})
...
ctc_arms = {}
ctc_specs = [
    ("baseline", {"CRISPASR_FC_DIRECT_SUBSAMPLE_CONV": "0"}, "ctc"),
    ("direct-conv0", {"CRISPASR_FC_DIRECT_SUBSAMPLE_CONV": "1"}, "ctc"),
    ("direct-dw", {"CRISPASR_FC_DIRECT_SUBSAMPLE_CONV": "2"}, "ctc"),
    ("direct-both", {"CRISPASR_FC_DIRECT_SUBSAMPLE_CONV": "3"}, "ctc"),
    ("flash-perhead", {"CRISPASR_FC_DIRECT_SUBSAMPLE_CONV": "0",
                       "CRISPASR_FC_GPU_MANUAL_ATTN": "0"}, "ctc"),
    ("flash-direct-both", {"CRISPASR_FC_DIRECT_SUBSAMPLE_CONV": "3",
                           "CRISPASR_FC_GPU_MANUAL_ATTN": "0"}, "ctc"),
    ("ffn-q8", {"CRISPASR_FC_DIRECT_SUBSAMPLE_CONV": "0"}, "ctc_ffn_q8_0"),
    ("ffn-f16", {"CRISPASR_FC_DIRECT_SUBSAMPLE_CONV": "0"}, "ctc_ffn_f16"),
]
for name, env, model_key in ctc_specs:
    kh.step(f"{name}.ctc")
    ctc_arms[name] = run_child("ctc", name, env, model_key)
results["models"]["ctc"] = ctc_arms

baseline = ctc_arms["baseline"]
implementation_arms = ["direct-conv0", "direct-dw", "direct-both", "flash-perhead", "flash-direct-both"]
stable = all(row["transcript_stable"] for arm in ctc_arms.values() for row in arm["clips"].values())
implementation_equal = all(
    ctc_arms[name]["clips"][clip]["texts"][-1] == baseline["clips"][clip]["texts"][-1]
    for name in implementation_arms for clip in clip_paths
)
precision_equal = {
    name: all(ctc_arms[name]["clips"][clip]["texts"][-1] == baseline["clips"][clip]["texts"][-1]
              for clip in clip_paths)
    for name in ("ffn-q8", "ffn-f16")
}
results["validation"] = {
    "passed": stable and implementation_equal,
    "all_arms_stable": stable,
    "implementation_equal": implementation_equal,
    "precision_equal": precision_equal,
}

(WORK / "results.json").write_text(json.dumps(results, indent=2) + "\n")
# Refresh the account-matched ccache dataset after this successful CUDA build.
ccache_dir = WORK / ".ccache"
if ccache_dir.is_dir():
    subprocess.check_call(["tar", "cf", str(WORK / "ccache.tar"), "-C", str(WORK), ".ccache"])
