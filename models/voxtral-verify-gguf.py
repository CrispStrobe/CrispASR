...
def main():
    ...
    # Tekken blobs
    for k in ("tokenizer.tekken.pattern", "tokenizer.tekken.specials",
              "tokenizer.tekken.vocab", "tokenizer.tekken.n_specials",
    ...
    if errors:
        ...
        print(f"    audio:    32 layers × 15 tensors = {32*15}  + 7 top-level = {32*15+7}")
        print(f"    proj:     2")
        print(f"    LLM:      30 layers × 9 tensors = {30*9}    + 3 top-level = {30*9+3}")
        print(f"    expected total: {32*15+7 + 2 + 30*9+3}, got {len(by_name)}")
        print(f"    KV metadata: {len(EXPECTED_KV)} keys verified")
        print(f"    Tekken blobs: 5/5 present")
        print(f"\n  PASS")
