...
// Pieces generated by Python `regex.findall` with the published patterns:
// Qwen2/3 (\p{N}) and Llama-3 / granite-4 (\p{N}{1,3}). Pins the \s+(?!\S) rule
// (a space run before a word leaves its last space to that word) and digit
// grouping - both differed from the tokenizers before the granite prompt fix.
TEST_CASE("qwen_pretokenize matches the Qwen2 / Llama-3 regex", "[bpe][pretokenize]") {
    ...
    for (const auto& c : cases) {
        INFO(c.in);
        CHECK(core_bpe::qwen_pretokenize(c.in) == c.qwen);
        CHECK(core_bpe::qwen_pretokenize(c.in, 3) == c.llama3);
    }
}
