...
static void ruby_whisper_log_callback(enum ggml_log_level level, const char* buffer, void* user_data) {
...
static VALUE ruby_whisper_s_log_set(VALUE self, VALUE log_callback, VALUE user_data) {
    ...
    if (!NIL_P(log_callback)) {
        VALUE finalize_log_callback =
            rb_funcall(mWhisper, rb_intern("method"), 1, rb_str_new2("finalize_log_callback"));
...
void Init_whisper() {
    ...
    rb_define_private_method(rb_singleton_class(mWhisper), "finalize_log_callback",
                             ruby_whisper_s_finalize_log_callback, 1);
