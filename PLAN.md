...
## §178 VoxCPM2 disclaimer voice-clone bug — DONE 2026-06-20

**Fixed `57549fba`.** `synthesize()` was reading `voice_path_` (set at
`init()` time) instead of `params.tts_voice` (set per call). The disclaimer
code calls `synthesize()` with `params.tts_voice.clear()` to request
zero-shot, but the backend ignored this — causing the disclaimer to be
voice-cloned, adding ~220 s per voice-clone call (242 s total for
"This audio was generated…" vs the expected ~21 s zero-shot).

Fix: `ref_path = params.tts_voice.empty() ? (consent ? "" : voice_path_) : params.tts_voice`.
When consent=true and tts_voice="" (disclaimer path), ref_path="", falling
through to zero-shot. Verified: "tokenized 'This audio was generated by
artificial intelligence.' -> 9 positions" (no "(incl ref)" tag).

Also added prefill timing diagnostics (VAE encode + locenc) guarded by
verbosity≥1. Root cause of first-run slow prefill (~188 s): Metal shader
JIT compilation during the per-patch locenc loop (69 patches × first call
triggers all kernel compilations). Warm-cache: ~1 s for 69 patches.

## §179 VoxCPM2 VAE encoder — Accelerate GEMM — DONE 2026-06-20

VAE encode of 11 s reference audio (176 k samples → 69 latent patches) was
taking 672 s on M1: the scalar nested loop `x_in[ic * T_in + it]` strides
T_in floats between consecutive ic accesses (up to 88 KB per step on block 3
with in_ch=512) → every inner ic access is a cache miss → effective
throughput ~0.14 GFLOP/s vs theoretical 12 GFLOP/s.

Fix: im2col + `cblas_sgemm` (Accelerate AMX, ~100 GFLOP/s on M1) in both
`causal_conv1d` (groups==1 path) and `vae_strided_conv1d`. For the
common ksize=1/stride=1/pad=0 case in residual units, `x_in` is used as
the B matrix directly — zero allocation.
