...
| **LOW** | [#127 Coverage gaps from 2026-05-26 sweep close-out](#127-coverage-gaps-from-the-2026-05-26-overlap-save-sweep-close-out) | Small | Three loose ends: (a) omniasr-llm overlap-save — **2026-06-13 Kaggle P100**: rc=0 on JFK (6.8s) + 60s concat (35.4s), consistent char counts short=long=chunk — runs to completion on GPU (no M1 timeout). Transcript extraction broken (log lines captured instead of text); needs `-np` sweep v4 to confirm actual output quality. (b) mimo-asr local test coverage in place since `2aeaf4c4` but doesn't run in CI because the 4.2 GB Q4_K doesn't fit the runner disk budget. (c) `cohere-asr-ja-v0.1` — **HF repo `cstr/cohere-asr-ja-v0.1-GGUF` returned 404**; either repo name is wrong or the GGUF was never published. Blocked until correct repo located. |
