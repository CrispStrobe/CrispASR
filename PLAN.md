...
| **HIGH** | [#57 Commercial-friendly TTS expansion](#57-commercial-friendly-tts-backend-expansion) | Phased | Phases 1-3 DONE; Turbo WORKING; F0 wired in; native voice cloning shipped → HISTORY §82; **#83 GPU drift fully bisected 2026-05-23 → LEARNINGS Round 8** — T3 round-4 kernel fixes (Q4_K×Q8_K + `_hp` mul_mm + PREC_F32) clean T3 on GPU; S3Gen UNet1D compound FP16 drift confirmed **cross-backend** (CUDA P100 cos_min=0.858 vs Metal 0.923; same 8 ops fix both: mul_mat/FA/norm/add/concat/gelu/tanh/softplus via `PIN_CPU_OP`). Metal default is full CPU (T3 dispatch overhead × 86 AR steps is 1.5× slower on M1). Remaining: promote UNet to FP32 on GPU (remove FP16 intermediate casts) as the proper fix replacing per-op CPU pinning, Kartoffelbox_Turbo DE |
