...
- **firered AED decoder — batch the beam (perf).** Shipped `e9d492a2`: linked OpenMP on the `firered-asr` target (it never did, so `cpu_matmul_bt`'s `#pragma omp parallel for` were no-ops → the AED beam decoder ran single-threaded) + a vectorizable float `cpu_dot` (replaced the scalar double-accumulate). Decoder **1443 → 365 ms/token (3.95×)**; issue19-5s 0.2→0.6× RT; issue19-35s 223 → 111 s (2× wall); transcripts byte-identical. Remaining ceiling is **memory bandwidth**: the beam path lazy-loads ~1.5 GB of F32 decoder weights and re-reads them for each of `beam_size` beams every token (thread-scaling 2→8 cores only 2.28×, so not compute-bound). **Next:** batch the active beams into one forward (stack their hidden vectors `[beam, d]`, read each weight matrix once), keeping per-beam self-attention separate — est. ~2-3× more, toward real-time. Alternative: route the beam path through the greedy path's Q4_K `ggml_vecmat` (4× less weight bandwidth, no F32 lazy-load). Files: `src/firered_asr.cpp` beam loop ~`2165-2400`.
- **#89 parakeet-ja auto-chunk window is too coarse (real-audio confirmed).** Reproduced the #89 drop on CUDA with **real** JA (`reazon_baseball_14s` ×3 = 42 s; HISTORY 2026-06-12): single-pass collapses past **~12 s** (1/3 repetitions); the 30 s auto-chunk fallback still loses content because the collapse happens *inside* each 30 s chunk (1/3); `--chunk-seconds 10` gets 2/3; **`--vad` recovers 3/3**. The earlier synthetic-TTS test was a false negative — clean audio doesn't trigger it (the bug is input-specific, as the maintainer noted). **Fix:** parakeet-ja's auto-chunk should use a ≤~12 s window (vs the shared 30 s `kLongAudioFallbackChunkSeconds` in `crispasr_long_audio_fallback.h`) or prefer VAD by default; `--vad` is the reliable workaround today. Related: this resolves the inverted reasoning behind the old #125 P2 "drop `CAP_UNBOUNDED_INPUT`" (`72b74486`) — that flag-drop is what caused the firered long-audio hang, reverted in `feba867d`.
