...
bool build_arm(Arm & a, ggml_type wt, Shape s, int n_threads,
               ggml_backend_buffer_type_t extra_buft, const char * label) {
    a.label   = label;
    a.backend = ggml_backend_cpu_init();
    ggml_backend_cpu_set_n_threads(a.backend, n_threads);

    ggml_backend_buffer_type_t wbuft =
        extra_buft ? extra_buft : ggml_backend_get_default_buffer_type(a.backend);
    ...
    if (!a.wbuf) { fprintf(stderr, "%s: weight alloc failed\n", label); return false; }
...
struct Stats { double best, med, mean; };
...
int main(int argc, char ** argv) {
    ...
    printf("ISA as ggml sees it: avx2=%d avx512=%d avx512_vnni=%d amx_int8=%d "
           ...
           ggml_cpu_has_avx2(), ggml_cpu_has_avx512(), ggml_cpu_has_avx512_vnni(),
           ggml_cpu_has_amx_int8(), ggml_cpu_has_neon(), ggml_cpu_has_dotprod(),
           ggml_cpu_has_matmul_int8(), ggml_cpu_has_sve());

    ggml_backend_dev_t dev = ggml_backend_dev_by_type(GGML_BACKEND_DEVICE_TYPE_CPU);
    if (!dev) { fprintf(stderr, "no CPU device\n"); return 1; }
    ...
    const struct { ggml_type t; const char * n; } types[] = {
        { GGML_TYPE_F32,  "f32"  },
        { GGML_TYPE_Q8_0, "q8_0" },
        { GGML_TYPE_Q4_0, "q4_0" },
        { GGML_TYPE_Q4_K, "q4_K" },
    ...
    for (Shape s : shapes) {
        printf("\n== shape K=%lld N=%lld M=%lld, threads=%d, reps=%d (interleaved) ==\n",
               (long long)s.K, (long long)s.N, (long long)s.M, n_threads, reps);
