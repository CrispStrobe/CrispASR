...
static bool fireredpunc_load(fireredpunc_context & ctx, const char * path) {
    ...
    fprintf(stderr, "fireredpunc: %dL, d=%d, ffn=%d, heads=%d, vocab=%d, labels=%d\n",
            ctx.n_layers, ctx.d_model, ctx.d_ffn, ctx.n_heads, ctx.vocab_size, ctx.n_classes);
...
static std::vector<int> fireredpunc_run(fireredpunc_context & ctx,
    ...
    for (int t = 0; t < N; t++) {
        ...
        for (int c = 1; c < ctx.n_classes; c++) {
            ...
            if (v > best_val) { best = c; best_val = v; }
        }
        preds[t] = best;
    }
...
char * fireredpunc_process(fireredpunc_context * ctx, const char * text) {
        ...
        if (ctx->tokenizer.is_sentencepiece) {
            ...
            if (it != ctx->tokenizer.token_to_id.end()) {
                ...
                while (start < lword.size()) {
                    ...
                    bool found = false;
                    while (end > start) {
                        std::string sub = (start == 0) ? lword.substr(0, end) : ("##" + lword.substr(start, end - start));
                        if (ctx->tokenizer.token_to_id.count(sub)) {
                            found = true;
                            start = end;
                            n_subtokens++;
    ...
    // Count Latin letters vs CJK characters to decide.
    {
        ...
        for (size_t i = 0; i < result.size(); i++) {
            ...
            else if (c >= 0xE0) cjk++; // rough: 3-byte+ UTF-8 = CJK/fullwidth
        }
        if (latin > cjk) {
            ...
            for (size_t i = 0; i < result.size(); ) {
                ...
                if (b0 >= 0xE0 && i + 2 < result.size()) {
                    ...
                    if (b0 == 0xEF && b1 == 0xBC && b2 == 0x8C) { mapped += ","; i += 3; continue; } // ，
                    if (b0 == 0xE3 && b1 == 0x80 && b2 == 0x82) { mapped += "."; i += 3; continue; } // 。
                    if (b0 == 0xEF && b1 == 0xBC && b2 == 0x9F) { mapped += "?"; i += 3; continue; } // ？
                    if (b0 == 0xEF && b1 == 0xBC && b2 == 0x81) { mapped += "!"; i += 3; continue; } // ！
                }
                mapped += result[i++];
            }
            result = mapped;
    ...
    for (size_t i = 0; i < fixed.size(); i++) {
        if (fixed[i] == 'i') {
            ...
            bool end = (i + 1 >= fixed.size() || fixed[i+1] == ' ' || fixed[i+1] == ',' ||
                       fixed[i+1] == '.' || fixed[i+1] == '?' || fixed[i+1] == '!');
